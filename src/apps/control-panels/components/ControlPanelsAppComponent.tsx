...
export function ControlPanelsAppComponent({
  ...
  const performReset = () => {
    ...
    const fileMetadataStore = localStorage.getItem("sanos:files");
    ...
    if (fileMetadataStore) {
      localStorage.setItem("sanos:files", fileMetadataStore);
    }
    if (usernameRecovery) {
  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      const timestamp = new Date()
        ...
        .join("-")
        .slice(0, -5);
      a.download = `sanOS-backup-${timestamp}.gz`;
  ...
  const performRestore = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        } else if (backup.localStorage && backup.localStorage["sanos:files"]) {
          // For newer backups, also check if files lack UUIDs
          try {
            const filesDataStr = backup.localStorage["sanos:files"];
        ...
        if (backup.indexedDB) {
          ...
          try {
            const db = await ensureIndexedDBInitialized();
            const persistedKey = "sanos:files";
            ...
            if (isOldBackupFormat && fileUUIDMap.size > 0) {
              ...
              localStorage.setItem(
                "sanos:indexeddb-uuid-migration-v1",
                "completed"
              );
    ...
    <>
      ...
      >
              ...
              <div className="space-y-4 h-full overflow-y-auto p-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[11px] text-gray-600 font-geneva-12">
                    ...
                    custom wallpapers. sanOS will restart after format.
                  </p>
                </div>
        ...
        <ConfirmDialog
          ...
          description="Are you sure you want to reset all settings? This will clear all saved settings and restore default states. sanOS will restart after reset."
        />
        <ConfirmDialog
          ...
          description="Are you sure you want to format the file system? This will permanently delete all documents (except sample documents), images, and custom wallpapers. sanOS will restart after format."
        />

...
const UUID_MIGRATION_KEY = "sanos:indexeddb-uuid-migration-v1";
...
export function useFileSystem(
  ...
  const formatFileSystem = useCallback(async () => {
    try {
      ...
      localStorage.removeItem("sanos:file-size-timestamp-sync-v1");
  ...
  useEffect(() => {
    const syncFileSizesAndTimestamps = async () => {
      ...
      const syncKey = "sanos:file-size-timestamp-sync-v1";

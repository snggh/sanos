...
export function LinkPreview({ url, className = "" }: LinkPreviewProps) {
  ...
  const isYouTubeUrl = (url: string): boolean => {
    return /(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/|os\.singgih\.dev\/ipod\/)/.test(
      url
    );
  };
  ...
  const extractYouTubeVideoId = (url: string): string | null => {
    try {
      ...
      // Handle os.singgih.dev/ipod/ links
      if (url.includes("os.singgih.dev/ipod/")) {
        const match = url.match(/os\.ryo\.dev\/ipod\/([^&\n?#]+)/);
  ...
  const getFaviconUrl = (url: string): string => {
    try {
      // For os.singgih.dev/ipod/ links, use YouTube favicon
      if (url.includes("os.singgih.dev/ipod/")) {
  ...
  useEffect(() => {
    const fetchMetadata = async () => {
      try {
        ...
        // Handle os.singgih.dev/ipod/ links specially - use YouTube metadata
        if (url.includes("os.singgih.dev/ipod/")) {
          const videoId = extractYouTubeVideoId(url);
          if (videoId) {
      ...
      {displayFullWidthThumbnail && metadata.image ? (
        ...
        <>
          ...
          <div className="px-2 pb-2">
            {isYouTubeUrl(url) ? (
              url.includes("os.singgih.dev/ipod/") ? (
